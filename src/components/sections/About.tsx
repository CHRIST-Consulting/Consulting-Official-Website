...
// Extract StatCard component to reduce nesting
...
const StatCard = ({ stat, counterValue, index }: StatCardProps) => {
...
// Extract CoreValueCard component to reduce nesting
interface CoreValueCardProps {
  value: typeof CORE_VALUES_DATA[0];
  index: number;
}
...
// Extract VideoModal component to reduce nesting
interface VideoModalProps {
  showVideo: boolean;
  onClose: () => void;
}

const VideoModal = ({ showVideo, onClose }: VideoModalProps) => {
...
// Extract VideoSection component to reduce nesting
interface VideoSectionProps {
  onVideoOpen: () => void;
}

const VideoSection = ({ onVideoOpen }: VideoSectionProps) => (
...
// Extract VisionMissionCard component to reduce nesting
interface VisionMissionCardProps {
  type: 'vision' | 'mission';
  delay: number;
}

const VisionMissionCard = ({ type, delay }: VisionMissionCardProps) => {
  const isVision = type === 'vision';
  const config = isVision 
    ? {
        icon: Target,
        title: 'Vision',
        content: 'Transform CHRIST Consulting into a highly profitable centre with global academic impact.',
        bgGradient: 'from-primary/10 to-yellow-400/10',
        iconGradient: 'from-primary to-royal-blue',
        titleColor: 'text-primary'
      }
    : {
        icon: TrendingUp,
        title: 'Mission', 
        content: 'Generate business growth and support CHRIST University revenues through innovation.',
  ...
  return (
    <div data-animate="true" data-delay={delay} className="relative group">
      <div className={`absolute inset-0 bg-gradient-to-br ${config.bgGradient} rounded-lg blur-sm group-hover:blur-none transition-all duration-300`}></div>
      <div className="relative bg-white/80 backdrop-blur-sm rounded-lg p-3 border border-white/40 hover:border-yellow-400/50 hover:shadow-lg hover:shadow-yellow-400/20 transition-all duration-300 hover:-translate-y-1">
        <div className="flex items-center gap-2 mb-2">
          <div className={`w-6 h-6 rounded-md bg-gradient-to-br ${config.iconGradient} flex items-center justify-center`}>
...
const About = () => {
  ...
  return (
    ...
    >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-1 gap-3">
              {STATS_DATA.map((stat, index) => (
                <StatCard 
                  key={stat.id}
                  stat={stat} 
                  counterValue={counters[stat.value as keyof typeof counters]} 
                  index={index} 
                />
              ))}
            </div>
          ...
          <div data-animate="true" data-delay="500" className="lg:col-span-1">
            <div className="space-y-4">
              {/* Compact Video */}
              <VideoSection onVideoOpen={handleVideoOpen} />

              {/* Compact Vision & Mission */}
              <div className="grid grid-cols-1 gap-3">
                <VisionMissionCard type="vision" delay={600} />
                <VisionMissionCard type="mission" delay={650} />
              </div>
            </div>
          </div>
        </div>
      ...
      <VideoModal showVideo={showVideo} onClose={handleVideoClose} />
    </section>
  );
};
